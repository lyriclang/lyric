...
- **Determinismus:** folgt aus der Spec: Arithmetik bitgenau, einzige nicht-deterministische Quellen (Uhr, Zufall aus dem OS, Umgebung, Dateien) sind capability-gated und im Runner nicht gewährt; Budget zählt Instruktionen, nicht Zeit. Ein Build ist damit reproduzierbar. (Offen: `std.random` ohne Seed nutzt eine OS-Quelle „capability-free“ (NativeRegistry.cs:524) — muss für comptime als `osAccess` gelten oder im Runner deterministisch geseedet werden. Anforderung an stdlib-redesign.)
...
Was fehlt: nicht-skalare Werte (Arrays/Structs), comptime als Attributwert, Backtrace-Notes in `LYR-CT0002`, Caching, `std.random`-Determinismus im Runner, LSP-Hover mit Wert.
